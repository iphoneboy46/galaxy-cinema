...
function BuyDay({
  ...
  useEffect(() => {
    MovieShowingData.map((mv) => {
      switch (rapName) {
        ...
        case "Galaxy Co.opXtra Linh Trung": {
          let rapFil = mv.galaxyND.find((time) => {
            if (time.value === suat) {
              return time.rap;
            }
  ...
  }, [movieName, rapName, day, suat, rapSo]);
    ...
    <div className="buy-movie">
      ...
      <div className="movie-select">
        {rapName !== "" ? (
          ...
          <Select placeholder="Chọn phim" options={NullList} />
        )}
      </div>
      <div className="movie-select">
        {movieName !== "" ? (
