...
function MenuResponsive() {
   ...
   useEffect(()=>{
    const link1 = document.querySelectorAll(".movie--showing-link") as any;
    const link2 = document.querySelectorAll(".menu-hover li a") as any;

    const menuRes = document.querySelector(".menu-res") as HTMLElement;
    link1.forEach((link:HTMLElement)=>{
        link.onclick = () =>{
            menuRes.style.display = "none";
        }
    })

    link2.forEach((link:HTMLElement)=>{
        link.onclick = () =>{
            menuRes.style.display = "none";
        }
    })
