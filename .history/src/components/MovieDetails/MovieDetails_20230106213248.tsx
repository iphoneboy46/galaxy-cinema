...
function MovieDetails({ name }: MovieDetailsProps) {
  ...
  const [soNgay, setSoNgay] = useState<string | number>("");
  const [soThang, setSoThang] = useState<string | number>("");
  const d = new Date();
  const ngay1 = d.getFullYear() + "-" + soThang + "-" + soNgay;
  const [n,setN] = useState<any>(ngay1);

  

  useEffect(() => {
    if (d.getDate().toString().length < 2 || d.getMonth().toString.length < 2) {
      let ngaySoFormat = `0${d.getDate()}`;
      let thangSoFormat = `0${d.getMonth() + 1}`;

      setSoNgay(ngaySoFormat);
      setSoThang(thangSoFormat);
    } else {
      let ngaySoFormat2 = d.getDate();
      let thangSoFormat2 = d.getMonth() + 1;

      setSoNgay(ngaySoFormat2);
      setSoThang(thangSoFormat2);
    }
  
    const ngay2 = d.getFullYear() + "-" + soThang + "-" + soNgay;
    setN(ngay2)
  },[n]);

  const [date, setDate] = useState<any>(n);
  ...
  const ng = today.getFullYear() + "-" + soThang + "-" + soNgay;
  console.log(ng);
