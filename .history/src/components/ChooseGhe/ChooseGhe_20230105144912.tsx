...
function ChooseGhe() {
  ...
  const [chooseK12, setChooseK12] = useState<boolean>(false);
  const [chooseK34, setChooseK34] = useState<boolean>(false);
  ...
  const handleK2 = (event: any) => {
    event.target.classList.toggle("selected")
    setChooseK34(true);
    if(event.target.classList.contains("ghe-doi")){
      setChooseK34(false);
    }

    if(event.target.classList.contains("ghe-doi selected")){
      setChooseK34(false);
    }
  ...
  return (
    <div className="choose-ghe">
      <h2>CHỌN GHẾ: {chooseK12 ? "k1,k2" : ""} {chooseK34 ? "k3,k4" : ""}
